...
function createHTMLEvent(index, events, storageName) {
    ...
    // Update location using dynamic storage key
    locationInput.addEventListener('change', () => {
        events[index].location = locationInput.value;
        chrome.storage.local.set({ [storageName]: events });
    });

    // Update title using dynamic storage key
...
// Load and render pending events (emails)
...
// Load and render highlighted events
...
async function approveEvent(index, events, storageName) {
    ...
    // Wait for animation to complete then remove event and create calendar event
    setTimeout(async () => {
